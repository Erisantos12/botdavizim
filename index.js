	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo se APRESENTe conforme a descrição do grupo *${mdata.subject}*\nPor favor não seja um ghost e se divirta ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} ainda bem que já foi 😋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Erik proibiu porno no gp👼'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* off:* o\n*WPP:* aqui mesmo\n*INSTA:* to sem aff kk\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate para  2d kakakak'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite familia ❤️'})
					break
				case 'belle':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK acabou de levar ban, trouxa  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
